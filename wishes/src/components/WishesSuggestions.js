import React from "react";
const birthdayWishes = [
  {
    wish:
      "Count your life by smiles, not tears. Count your age by friends, not years. Happy birthday!",
  },
  {
    wish:
      "“Happy birthday! I hope all your birthday wishes and dreams come true.",
  },
  {
    wish:
      "A wish for you on your birthday, whatever you ask may you receive, whatever you seek may you find, whatever you wish may it be fulfilled on your birthday and always. Happy birthday!",
  },
  {
    wish:
      "Another adventure filled year awaits you. Welcome it by celebrating your birthday with pomp and splendor. Wishing you a very happy and fun-filled birthday!",
  },
  {
    wish:
      "May the joy that you have spread in the past come back to you on this day. Wishing you a very happy birthday!",
  },
  {
    wish:
      "Happy birthday! Your life is just about to pick up speed and blast off into the stratosphere. Wear a seat belt and be sure to enjoy the journey. Happy birthday!",
  },
  {
    wish:
      "This birthday, I wish you abundant happiness and love. May all your dreams turn into reality and may lady luck visit your home today. Happy birthday to one of the sweetest people I’ve ever known.",
  },
  {
    wish:
      "May you be gifted with life’s biggest joys and never-ending bliss. After all, you yourself are a gift to earth, so you deserve the best. Happy birthday.",
  },
];

const graduatedWishes = [
  {
    wish: "Congratulations and BRAVO!",
  },
  {
    wish: "This calls for celebrating! Congratulations!",
  },
  {
    wish:
      "The community, the church, your family and especially your mama could not be prouder of you.",
  },
  {
    wish:
      "You deserve all KINDS of high fives and hugs! You worked hard and got it done.",
  },
  {
    wish:
      "If you’re wondering how the last four years went so fast, you should see how quickly the rest of life flies by. Enjoy this moment. Congratulations.",
  },
  {
    wish:
      "Just remember that like college, all things that are worth it take time and effort. Continue to follow your dreams.",
  },
  {
    wish:
      "Congratulations on graduating! This is just the beginning; you are a star in the making. Shine on.",
  },
  {
    wish:
      "You are an achiever. You have made us all proud; keep up the good work. Congratulations on graduation.",
  },
];

const weeddingWishes = [
  {
    wish: "Wishing you a lifetime of love and happiness",
  },
  {
    wish: "Your wedding day will come and go, but may your love forever grow.",
  },
  {
    wish:
      "Best wishes on this wonderful journey, as you build your new lives together.",
  },
  {
    wish:
      "Thank you for letting us/me share in this joyful day. We/I wish you all the best as you embark on this wonderful union.",
  },
  {
    wish:
      "Wishing you joy, love and happiness on your wedding day and as you begin your new life together.",
  },
  {
    wish: "Wishing you the best today and always.",
  },
  {
    wish: "Congratulations on your marriage, and welcome to the family!",
  },
  {
    wish:
      "Today, we add one more member to our family, and we couldn't be happier. Best wishes to you both",
  },
];
function WishesSuggestions({ category,setWishesSuggestionsClicked,setWish }) {
  switch (category) {
    case "wedding":
      return <div className="wishesSuggestions">
          <img src="/exit.png" alt=""/>
{weeddingWishes.map(wish=>{
    return (<p onClick={e=>{
        setWish(wish.wish)
        setWishesSuggestionsClicked(false)

    }
    }>{wish.wish}</p>)
})}

      </div>;
    case "birthday":
        return <div className="wishesSuggestions">
           <div> <label></label><img src="/exit.png" alt="" onClick={e=>{
           setWishesSuggestionsClicked(false)
           }
           }/></div>
            
            {birthdayWishes.map(wish=>{
          return (<p
              onClick={e=>{
                  setWish(wish.wish)
          setWishesSuggestionsClicked(false)
  
          
              }
          }
          >{wish.wish}</p>)
      })}</div>;

      case "graduation":
        return <div className="wishesSuggestions">
           <div> <label></label><img src="/exit.png" alt="" onClick={e=>{
           setWishesSuggestionsClicked(false)
           }
           }/></div>
            
            {graduatedWishes.map(wish=>{
          return (<p
              onClick={e=>{
                  setWish(wish.wish)
          setWishesSuggestionsClicked(false)
  
          
              }
          }
          >{wish.wish}</p>)
      })}</div>;

    default:
        return <div className="wishesSuggestions">no wishes Available</div>;
        

  }
}

export default WishesSuggestions;
